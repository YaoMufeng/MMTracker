...
from dwconv_models import CRSiameseDWConv
from correlation_models import CRSiameseDWResConvCorrelation1
from models import load_model

sys.path.append("/home/ymf/codes")
...
# _evaluator = MCMOTEvaluatorBiasGMM
# _evaluator = MCMOTEvaluatorBias
_evaluator = MCMOTEvaluatorBiasWarp
# _evaluator = MCMOTEvaluatorBiasPublic
...
def main(exp, args, num_gpu):
    ...
    #save_folder = "track_results_centerdis_nofuse"
    ...
    #     depth,width,num_classes = 0.33,0.50,10
    #     backbone = YOLOPAFPN(depth, width, in_channels=in_channels)
    #     head = YOLOXHead(num_classes, width, in_channels=in_channels,args=args)
    #     model = YOLOX2(backbone, head)
    ...
    # checkpoint = "checkpoints/pwcnet_ft_4x1_300k_sintel_final_384x768.pth"

    config = "configs/flownet/flownets_8x1_sfine_sintel_384x448.py"
    checkpoint = "checkpoints/flownets_8x1_slong_flyingchairs_384x448.pth"

    # config = "checkpoints/flownet2cs_8x1_sfine_flyingthings3d_subset_384x768.py"
    # checkpoint = "checkpoints/flownet2cs_8x1_sfine_flyingthings3d_subset_384x768.pth"



    # flow_root = "/home/ymf/codes/mmflow"
    # _config = os.path.join(flow_root,config)
    # _checkpoint = os.path.join(flow_root,checkpoint)
       
    # flow_model = init_model(_config, _checkpoint, device="cuda")
    # flow_model = flow_model.eval()


    # models_folder = "/home/ymf/codes/flow_estimation/weights"
    # snap_to_load = "test_CRSiameseDWResConvCorrelation1_512x384_distill_pwcnet_exp-lr_0.004_stage0_best"
    # model_load_path = os.path.join(models_folder,snap_to_load)
    
    # flow_model = CRSiameseDWResConvCorrelation1(num_classes=2).cuda()
    # flow_model = nn.DataParallel(flow_model).cuda()
    # flow_model = load_model(model_load_path,flow_model)
    # flow_model = flow_model.eval()
    ...
    *_, summary = evaluator.evaluate(
    ...
    #     ts[k] = v2
    #pdb.set_trace()
    from yolox.evaluators.mcmot_eval import clean_gt_and_ts
