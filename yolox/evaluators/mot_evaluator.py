...
class MOTEvaluator:
    ...
    def __init__(
        self, args, dataloader, img_size, confthre, nmsthre, num_classes,interested_seqs=None):
        """
        Args:
    ...
    def evaluate(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(progress_bar(self.dataloader)):
            ...
            if self.interested_seqs is not None and video_name not in self.interested_seqs:
                #print("need continue: ",video_name)
            ...
            with torch.no_grad():
                ...
                outputs,class_prob = postprocess_prob(outputs, self.num_classes, self.confthre, self.nmsthre)
                
                # print(class_prob.min(),class_prob.max())
                # print("="*10)
                # print(outputs)
                # print("-"*10)
            ...
            if outputs[0] is not None:
                online_targets = tracker.update(outputs[0], info_imgs, self.img_size,class_prob)
                online_tlwhs = []
                online_ids = []
                online_scores = []
                online_class_ids = []
    ...
    def evaluate_deepsort(
        ...
        tracker = DeepSort(model_folder, min_confidence=self.args.track_thresh,data_root=data_root)
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            for t in online_targets:
                ...
                if True:
                    online_tlwhs.append(tlwh)
                    online_ids.append(tid)
                    online_class.append(_class)
                    scores.append(1)
            # save results
            results.append((frame_id, online_tlwhs, online_ids,scores,online_class))
